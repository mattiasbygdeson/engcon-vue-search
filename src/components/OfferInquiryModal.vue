    ...
    <article class="share-modal__container">
      ...
      <main class="share-modal__content">
        <form v-on:submit.prevent="sendEmail">
          ...
          <input id="input-name" class="input-name" type="text" v-model="senderName" />
  ...
  methods: {
    ...
    async sendEmail() {
      ...
      for(var i = 0; this.favorites.length > i; i++) {
        productList += 
          "<p style='border-top:1px solid #ccc;font-size:1.2em;font-weight:bold;margin-bottom:0;'>" + this.favorites[i].title + "</p>" +
      ...
      let res = await sendEmail(msg, pricelistLink, footer, recipent, subject);
